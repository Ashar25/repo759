...
int main(int argc, char *argv[]) {
    ...
    for (int step = 0; step < Nt; step++) {
        ...
        // TODO: Ensure particles stay within the board limits
            pos[i][0] = std::min(board_size, std::max(-board_size, pos[i][0]));
            pos[i][1] = std::min(board_size, std::max(-board_size, pos[i][1]));
            pos[i][2] = std::min(board_size, std::max(-board_size, pos[i][2]));
        }
